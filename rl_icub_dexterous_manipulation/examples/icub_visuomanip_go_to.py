...
import os
import sys
sys.path.append("./rl-icub-dexterous-manipulation/rl_icub_dexterous_manipulation/external/DenseFusion")
sys.path.append("./rl-icub-dexterous-manipulation/rl_icub_dexterous_manipulation/examples")
import numpy as np
...
import torch
from rl_icub_dexterous_manipulation.external.d3rlpy_mod.d3rlpy.algos.awac import AWAC
...
def print_args_in_rows(args):
  """Prints arguments in a two-column format.

  Args:
    args: The parsed arguments namespace from argparse.
  """
  max_key_len = max(len(key) for key in args.__dict__)  # Find longest argument key length
  for key, value in args.__dict__.items():
    print(f"{key:<{max_key_len}}: {value}")  # Left-align key, format value
...
if args.task == 'reaching':
    iCub = ICubEnvReaching(
                           done_if_joints_out_of_limits=False, # lo agregué
    ...
    iCub = ICubEnvRefineGrasp(task_kwargs=task_kwargs)
