...
def make_env(env_kwargs, seed=0, training=True):
...
def main(_):
    ...
    run = wandb.init(
                    job_type = "train", 
                    dir = Path.cwd()/FLAGS.log_root, 
                    config = env_kwargs, 
                    project = "reaching_task", 
                    # entity = None, 
                    # reinit = None, 
                    tags = [str(FLAGS.seed),str(FLAGS.learning_rate)], 
                    group = "experiment_1", 
                    # name: str | None = None, 
                    # notes: str | None = None, 
    ...
    eval_env = make_env(env_kwargs, seed=FLAGS.eval_seed, training=False)
    ...
    wandb_callback = WandbCallback(
        verbose = 2, 
        model_save_path = Path.cwd()/FLAGS.log_root/"wandb_model", 
