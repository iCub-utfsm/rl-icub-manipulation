...
from rl_icub_dexterous_manipulation.utils.vgn_utils import VGNEstimator
...
class ICubEnvRefineGrasp(ICubEnv):
    ...
    def step_cartsolv(self):
        ...
        for actuator_id, actuator_dict in enumerate(self.actuators_dict):
            if actuator_id in self.actuators_to_control_ik_ids:
                ...
                for j in range(len(actuator_dict['jnt'])):
                    self.target[actuator_id] += qpos_i[self.joint_ids_icub_dict[actuator_dict['jnt'][j]]] * \
                                            actuator_dict['coeff'][j]

        self.do_simulation(self.target, self.frame_skip, increase_steps=True)
        ...
        joints_vel = np.empty([0, ], dtype=np.float32)
        joints_acc = np.empty([0, ], dtype=np.float32)
        joints_torque = np.empty([0, ], dtype=np.float32)
        named_qpos = self.env.physics.named.data.qpos
        named_qvel = self.env.physics.named.data.qvel
        named_qacc = self.env.physics.named.data.qacc
        gear_ratios = self.env.physics.named.model.actuator_gear #[:,0]
        motor_torques = self.env.physics.named.data.actuator_force #.tolist()
        for actuator_id, actuator_dict in enumerate(self.actuators_dict):
            if actuator_id in self.actuators_to_control_ik_ids:
                for j in range(len(actuator_dict['jnt'])):
                    joints_pos = np.append(joints_pos, named_qpos[actuator_dict['jnt'][j]] * \
                                            actuator_dict['coeff'][j])
                    joints_vel = np.append(joints_vel, named_qvel[actuator_dict['jnt'][j]] * \
                                            actuator_dict['coeff'][j])
                    joints_acc = np.append(joints_acc, named_qacc[actuator_dict['jnt'][j]] * \
                                            actuator_dict['coeff'][j])
                    # joints_torque = np.append(joints_torque, gear_ratios[actuator_dict['jnt'][j]] * \
                    #                         motor_torques[[actuator_dict['jnt'][j]]])
        
        if self.prev_action is None:
            self.prev_action = qpos_i[self.actuators_to_control_ik_ids]

        reward = self._get_reward_v2(qpos_i[self.actuators_to_control_ik_ids], self.prev_action,
    ...
    def step(self, action, increase_steps=True, pre_approach_phase=False):
        ...
        # reward = self._get_reward_v1(current_action, self.prev_action, 
        ...
        info = {'Steps': self.steps,
                ...
                'reward': reward,
                'is_success': done_goal_eef} # or done_goal_joints
    ...
    def _get_reward_v1(self, action, prev_action, 
    ...
    def _get_reward_v2(self, 
        ...
        reward = 0
        # Current eef position close to target position
        reward += -1.0 * np.linalg.norm(current_eef_pose - desired_eef_pose)
        # Regularization term on actions
        reward += -0.1 * np.linalg.norm(action - prev_action)
        # Reachead target eef position
        if done_goal_eef:
            return 1.0
        # # Max episode timesteps reached
